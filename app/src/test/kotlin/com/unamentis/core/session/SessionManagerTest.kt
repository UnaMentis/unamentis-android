...
class SessionManagerTest {
    ...
    @Test
    fun `startSession transitions to IDLE and starts audio capture`() =
        testScope.runTest {
    ...
    @Test
    fun `startSession with curriculum loads curriculum`() =
        testScope.runTest {
    ...
    @Test
    fun `stopSession stops audio and finalizes session`() =
        testScope.runTest {
            sessionManager.startSession()
            advanceUntilIdle()
    ...
    @Test
    fun `pauseSession transitions to PAUSED and stops audio`() =
        testScope.runTest {
            sessionManager.startSession()
            advanceUntilIdle()
    ...
    @Test
    fun `resumeSession transitions back to IDLE and restarts audio`() =
        testScope.runTest {
            sessionManager.startSession()
            advanceUntilIdle()
    ...
    @Test
    fun `sendTextMessage adds to transcript and triggers LLM`() =
        testScope.runTest {
            sessionManager.startSession()
            advanceUntilIdle()
    ...
    @Test
    fun `conversation flow transitions through states correctly`() =
        testScope.runTest {
            sessionManager.startSession()
            advanceUntilIdle()
            ...
            every { vadService.processAudio(any()) } returns
                VADResult(
                    isSpeech = true,
                    confidence = 0.9f,
                )
    ...
    @Test
    fun `metrics track latency correctly`() =
        testScope.runTest {
            sessionManager.startSession()
            advanceUntilIdle()
    ...
    @Test
    fun `cannot start session when already active`() =
        testScope.runTest {
            sessionManager.startSession()
            advanceUntilIdle()
    ...
    @Test
    fun `transcript entries have correct timestamps`() =
        testScope.runTest {
            sessionManager.startSession()
            advanceUntilIdle()
    ...
    @Test
    fun `session tracks total turns`() =
        testScope.runTest {
            sessionManager.startSession()
            advanceUntilIdle()
    ...
    @Test
    fun `empty transcription returns to IDLE`() =
        testScope.runTest {
            sessionManager.startSession()
            advanceUntilIdle()

            every { sttService.startStreaming() } returns
                flowOf(
